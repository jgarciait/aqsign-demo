...
export default function EnhancedCaseFileDocuments({ 
  ...
  const loadDocuments = useCallback(async (skipLoading = false) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('📄 LoadDocuments - Uncategorized documents:', uncategorized.map(d => ({ id: d.id, name: d.file_name, category_id: d.category_id, isUncategorized: d.isUncategorized })))
  ...
  const handleMoveDocument = async (documentId: string, categoryId: string | null) => {
    ...
    console.log('Moving document:', documentId, 'to category:', categoryId, 'from category:', documentToMove.category_id)
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('🔄 Force refreshing data after successful move...')
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription className="space-y-2">
                <p>¿Estás seguro que deseas eliminar la carpeta <strong>"{categoryToDelete?.name}"</strong>?</p>
        ...
        <Dialog open={showDocumentModal} onOpenChange={setShowDocumentModal}>
          <DialogContent className="max-w-4xl max-h-[80vh]">
            <DialogHeader>
              ...
              <DialogDescription>
                Vista previa del documento y sus firmas
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              {selectedDocument && (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm mt-2">
                    Tamaño: {formatFileSize(selectedDocument.file_size)} • 
                    Creado: {new Date(selectedDocument.created_at).toLocaleDateString()}
                  </p>
                  {selectedDocument.hasSigned && (
