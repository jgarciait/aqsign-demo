...
export default function EnhancedSignatureModal({ 
  ...
  // Process Wacom signature to make background transparent
  const processWacomSignature = (imageDataUrl: string): Promise<string> => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        if (!ctx) {
          resolve(imageDataUrl) // Return original if processing fails
          return
        }
        ...
        // Convert white pixels to transparent (same logic as canvas signature)
  ...
  useEffect(() => {
    ...
    const handleWacomMessage = async (event: MessageEvent) => {
      if (event.data?.type === 'wacom-sign' && event.data?.imageSign) {
        ...
        const processedSignature = await processWacomSignature(event.data.imageSign)
