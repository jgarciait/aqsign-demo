...
export default async function DocumentDetailPage({ params }: { params: { id: string } }) {
  console.log("DocumentDetailPage: Starting with ID:", params.id)
          ...
          <dl className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div>
              ...
              <dd className="mt-1 text-sm text-gray-900">
                {request.sent_at ? (
                  <>
                    {format(new Date(request.sent_at), "PPP 'a las' p")}
            ...
            {request.received_at && (
              <div>
                ...
                <dd className="mt-1 text-sm text-gray-900">
                  {format(new Date(request.received_at), "PPP 'a las' p")}
                  <span className="text-gray-500 ml-1">
                ...
                <dd className="mt-1 text-sm text-gray-900">
                  <div className="flex items-center">
                    ...
                    <span className="font-semibold signed-text-glow">
                      {format(new Date(request.signed_at), "PPP 'a las' p")}
                    </span>
                    <span className="text-gray-500 ml-1">
                      ({formatDistanceToNow(new Date(request.signed_at), { addSuffix: true })})
                    </span>
                  </div>
                </dd>
              </div>
              ...
              <div>
                ...
                <dd className="mt-1 text-sm text-gray-900">
                  <div className="flex items-center">
                    <CheckCircle className="h-4 w-4 text-green-500 mr-1 signed-text-glow" />
                    <span className="font-semibold signed-text-glow">
                      {format(new Date(request.returned_at), "PPP 'a las' p")}
                    </span>
                    <span className="text-gray-500 ml-1">
                      ({formatDistanceToNow(new Date(request.returned_at), { addSuffix: true })})
                    </span>
                  </div>
                </dd>
              </div>
