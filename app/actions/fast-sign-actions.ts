...
export async function getFastSignDocumentsByArchiveStatus(
  ...
  try {
    ...
    let query = supabase
      ...
      .in("document_type", ["email", "fast_sign"])
      .order("created_at", { ascending: false })
...
export async function archiveFastSignDocument(documentId: string) {
  ...
  try {
    ...
    console.error("Error in archiveFastSignDocument:", error)
    return {
      error: error instanceof Error ? error.message : "An unexpected error occurred",
    }
  }
...
export async function unarchiveFastSignDocument(documentId: string) {
  ...
  try {
    ...
    console.error("Error in unarchiveFastSignDocument:", error)
    return {
      error: error instanceof Error ? error.message : "An unexpected error occurred",
    }
  }
...
export async function deleteFastSignDocument(documentId: string) {
  ...
  try {
    ...
    if (document.file_path) {
      ...
      // Extract the path within the bucket - handle different path formats
      ...
      // If the path starts with "uploads/", use that directly
      ...
      if (storageError) {
        ...
        if (!deleteData || deleteData.length === 0 || !deleteData[0]) {
      ...
      console.warn("No file_path found for document, skipping storage deletion")
    }
